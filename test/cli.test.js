...
test('[sumoStream]', (t) => {
  ...
  /* --query */
  file.sumoStream(auth, { flags: { query: 'error' } });
  ...
  t.equal(args[1].to - args[1].from, 15 * 60 * 1000, `[${count}/${total}] difference between \'from\' and \'to\' properties should be 10m`);
  count++;
  ...
  args = createReadStream.getCall(5).args;
