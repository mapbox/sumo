  ...
  pollStatus() {
    this.checkStatus().then(() => {
      if (this.status.state === 'DONE GATHERING RESULTS') {
        if (this.status.messageCount === 0) this.empty = true;
  ...
  checkStatus(attempts) {
    ...
    return this.onceCreated()
      ...
      .then((status) => {
        ...
        if (status.recordCount > 0) this.emit('records');
        return status;
      });
  }
    ...
    const request = (attempts) => {
      ...
      return this.waitForResults(type)
        ...
        .then((response) => {
          ...
          return response.body[`${type}s`].map((result) => {
            this[`${type}s`].push(result.map);
            return result.map;
          });
        })
        .catch((err) => {
